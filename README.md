...
59 tests passing, plus an enhanced interpretive mesh demo. 1,452 lines of Fard across 26 source files.
...
- Weighted collapse: collapse decisions derived from reputation-weighted witness scores
- Zero-weight witnesses are inert; cross-space reputation does not bleed into collapse
- Context-scoped reputation live
- Semantic challenge resolution live
- Executable and inference claims: signed, verified, tamper-evident, convertible to standard claims
...
- Live node process: HTTP server with SQLite-backed persistent state, survives restarts
- Convergence layer: partition heal, bidirectional sync, no claims silently dropped, claim-sets converge identically on both nodes
